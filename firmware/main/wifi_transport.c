 ...
 * Azure Container Apps serves certificates signed by this root.
 * Required for HTTPS POSTs (Mode 3 — Azure).
 * Not used for plain HTTP (Mode 1 — LAN); esp_http_client ignores it.
 *
 * If the server certificate chain changes (e.g. migration away from
 * Let's Encrypt), replace this cert and reflash all devices.
 */
...
static esp_err_t wifi_post(const char *url, const char *body, size_t len)
{
    esp_http_client_config_t cfg = {
        ...
        .cert_pem = CA_CERT_PEM,   /* used for https:// URLs; ignored for http:// */
    };
